...
3. [주요 기능](#주요-기능)
4. [시작하기](#시작하기)
5. [Clerk + Supabase 통합 가이드](#clerk--supabase-통합-가이드)
...
- `/integration-test`: Clerk + Supabase 네이티브 통합 종합 테스트 (서버/클라이언트/Server Action)
...
Clerk와 Supabase를 최신 모범 사례에 따라 통합하는 방법은 다음 문서를 참고하세요:

📖 **[상세 통합 가이드 보기](./docs/CLERK_SUPABASE_INTEGRATION.md)**

### 주요 내용

- ✅ **네이티브 통합 설정** (2025년 권장 방식)
- ✅ **Supabase Third-Party Auth Provider 설정**
- ✅ **클라이언트/서버 컴포넌트별 사용법**
- ✅ **Server Actions 통합**
- ✅ **RLS 정책 설정 가이드**
- ✅ **문제 해결 가이드**
...
1. **Supabase에서 Clerk를 Third-Party Auth Provider로 추가**
   - Supabase Dashboard → Authentication → Sign In / Up → Third Party Auth
   - Clerk 선택 후 Clerk Dashboard에서 제공하는 도메인 입력

2. **Clerk에서 Supabase 통합 활성화**
   - Clerk Dashboard → Setup → Supabase
...
자세한 내용은 [통합 가이드 문서](./docs/CLERK_SUPABASE_INTEGRATION.md)를 참고하세요.

## 추가 설정 및 팁

### Clerk 한국어 설정

프로젝트에 이미 Clerk 한국어 로컬라이제이션이 적용되어 있습니다. `app/layout.tsx`의 `ClerkProvider`에서 한국어 로컬라이제이션이 설정되어 있습니다.

**로컬라이제이션 설정 위치**: `lib/clerk-localization.ts`

특정 텍스트를 커스터마이징하려면 `lib/clerk-localization.ts` 파일을 수정하세요.

📖 **[상세 로컬라이제이션 가이드 보기](./docs/CLERK_LOCALIZATION.md)**
